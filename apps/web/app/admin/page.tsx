...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {statsQuery.isLoading && <p className="text-sm text-ink-muted">Memuat statistik...</p>}
      {statsQuery.isError && <p className="text-sm text-red-400">Gagal memuat statistik dashboard.</p>}

      {statsQuery.data && (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          <StatCard label="Total Pendapatan" value={formatRupiah(statsQuery.data.revenue)} hint="Dari pesanan yang sudah dibayar" />
          <StatCard label="Total Pesanan" value={String(statsQuery.data.total_orders)} />
          <StatCard label="Total Produk" value={String(statsQuery.data.total_products)} />
          <StatCard label="Total Cabang" value={String(statsQuery.data.total_branches)} />
          <StatCard label="Total Staf" value={String(statsQuery.data.total_staff)} />
        </div>
      )}

      {statsQuery.data && statsQuery.data.low_stock_count > 0 && (
        <div className="rounded-2xl border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-300">
