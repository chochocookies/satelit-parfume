// Package memberships computes a customer's loyalty tier from their
...
//
// This package's own original scaffold (Phase 1's doc.go, now folded in
// here) described "membership tiers and points ledger" — a separate,
// earnable/spendable points balance on top of the tier itself. Only the
// tier half is built: a points system that customers spend is a
// checkout-touching feature much like promo codes (crediting and
// debiting a balance atomically with the order it applies to), and
// bundling it into this same pass risked exactly the kind of scope
// creep this session has otherwise been careful to avoid. Worth its own
// follow-up pass rather than folding in here as an afterthought.
//
// The tier half that IS built is deliberately read-only and derived,
// not a stored, mutable balance: tier is recalculated from orders.total
// every time it's asked for, rather than tracked as its own running
// total that could drift out of sync with the orders it's supposed to
// reflect. That also means this package never needs to touch Checkout
// at all — same scope discipline internal/reviews' own doc comment
// explains for why it doesn't touch products.Repository.List either.
package memberships
